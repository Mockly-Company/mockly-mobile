...
const WelcomeComponent = () => {
  return (
    <View style={tw`bg-slate-50 dark:bg-gray-900`}>
      {/* 대형 헤더 */}
      <View
        style={tw`w-full items-center pt-12 pb-6 bg-white dark:bg-gray-800 border-b border-slate-200 dark:border-gray-700 mb-8`}>
        <Text
        ...
        <Text
          style={tw`text-base text-slate-600 dark:text-gray-300 max-w-xl text-center`}>
      ...
      <View style={tw`w-full items-center mb-10`}>
        <Text style={tw`text-xl font-bold mb-5 text-slate-800 dark:text-white`}>
          다크/라이트 모드 미리보기
        </Text>
        <View style={tw`flex-row justify-center items-end gap-10`}>
          <View style={tw`items-center`}>
            <Text
              style={tw`text-sm font-bold mb-2 text-gray-900 dark:text-white`}>
              라이트 모드
            </Text>
            {/* @ts-ignore: web video tag for storybook */}
            <video
              src="/라이트모드.mp4"
              style={{
                width: 180,
      ...
      {/* 본문 컨텐츠 래퍼 */}
      <View
        style={tw`w-full max-w-2xl self-center p-8 bg-slate-50 dark:bg-gray-800 rounded-2xl mb-10`}>
        {/* 주요 특징 */}
        <View
        ...
        {/* 빠른 이동 버튼 */}
        <View style={tw`flex-row justify-center gap-6 mb-8`}>
          <TouchableOpacity
            style={tw`items-center mx-3 bg-blue-50 dark:bg-blue-900 rounded-lg p-3 min-w-30`}
...
export const 디자인_시스템: Story = {render: () => <WelcomeComponent />};
